...
/**
 * Set a solar deal's financing product.
 *
 * On Solar, "deal type" IS the financing product — there is no insurance-vs-cash
 * distinction, because there is no insurer. Switching product re-runs the same
 * save path as the Financing tab, so the columns that do not apply to the new
 * product are cleared rather than left holding stale figures from the old one.
 */
export async function setSolarFinanceProductAction(input: z.infer<typeof productSchema>) {
  const user = await requireUser();
  const parsed = productSchema.safeParse(input);
  if (!parsed.success) return fail("Invalid product.");

  const existing = await prisma.solarFinance.findUnique({
    where: { leadId: parsed.data.leadId },
    select: {
      grossPpwCents: true, dealerFeePct: true, adderTotalCents: true,
      rateMillsPerKwh: true, monthlyPaymentCents: true, escalatorPct: true,
      termYears: true, aprPct: true, loanTermMonths: true,
    },
  });

  return saveSolarFinanceAction({
    leadId: parsed.data.leadId,
    product: parsed.data.product,
    grossPpwCents: existing?.grossPpwCents || undefined,
    dealerFeePct: existing?.dealerFeePct ?? undefined,
    adderTotalCents: existing?.adderTotalCents ?? undefined,
    rateMillsPerKwh: existing?.rateMillsPerKwh ?? null,
    monthlyPaymentCents: existing?.monthlyPaymentCents ?? null,
    escalatorPct: existing?.escalatorPct ?? null,
    termYears: existing?.termYears ?? null,
    aprPct: existing?.aprPct ?? null,
    loanTermMonths: existing?.loanTermMonths ?? null,
  });
}
