...
export default async function TeamPage() {
  ...
  // Roles this user is actually allowed to invite into (Super Admin-only roles
  // are hidden for admins and sales managers).
  const inviteRoles = assignableRolesFor(user.role).map((r) => ({ value: r, label: roleLabel(r) }));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={canInvite ? <TeamInvite roles={inviteRoles} /> : undefined}
      />
