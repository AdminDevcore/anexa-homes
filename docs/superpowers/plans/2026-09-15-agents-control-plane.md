...
- **Order of work:** Tasks 0–8, 10–22, 23, 24, Task 25 for deploy 1; then Task 9, Tasks 23 and 24 again, and Task 25 for deploy 2. Task 9's migration is `20260916100300_action_required_stages`, so it sorts after the other two.
...
**Rebased onto origin/main `ed3b832` (2026-09-15):** migrations renamed to `20260916100000` / `20260916100200` / `20260916100300` so they sort after main's `20260915130000_contract_evidence_classification`.

**Rebased onto origin/main `d694e57` (2026-09-15):** migrations renamed to `20260916100000` / `20260916100200` / `20260916100300` so they sort after main's `20260916090000_payroll_items_commission_id_index`; main's `pipeline/stage-guard.ts` and its CI test are now on the base.
...
| `prisma/migrations/20260916100000_agents_control_plane/` | Enums, tables, notification event values (generated). |
| `prisma/migrations/20260916100200_agents_seed_rows/` | Hello Agent and starter notification rules. |
| `prisma/migrations/20260916100300_action_required_stages/` | Stage flags and three roofing stages. Deploy 2. |
...
- Create: `prisma/migrations/20260916100000_agents_control_plane/migration.sql` (generated)
...
```bash
cd /Users/mustafajoulani/Desktop/anexa-agents-wt
F=prisma/migrations/20260916100000_agents_control_plane/migration.sql
mkdir -p "$(dirname "$F")"
...
Expected: `Applying migration `20260916100000_agents_control_plane``, `All migrations have been successfully applied.`, `✔ Generated Prisma Client`.
...
```bash
cd /Users/mustafajoulani/Desktop/anexa-agents-wt
git add prisma/schema.prisma prisma/migrations/20260916100000_agents_control_plane/migration.sql
...
**Files:**
- Create: `prisma/migrations/20260916100300_action_required_stages/migration.sql`
...
const MIGRATION = join(
  __dirname,
  "../../../../../prisma/migrations/20260916100300_action_required_stages/migration.sql"
);
...
describe("20260916100300_action_required_stages", () => {
...
Expected: FAIL in `beforeAll` — psql: `could not open file ".../20260916100300_action_required_stages/migration.sql"`.
...
const STAGES = [
  ...
  // nowhere else. Same rows as migration 20260916100300_action_required_stages.
...
Expected: `Applying migration `20260916100300_action_required_stages``; `✅ Seed complete.`; 20 rows `0|new_lead|f` … `19|cancelled|f`, with `t` on exactly `claim_denied` (5), `supplement_needed` (7), `qc_failed` (13) and `payment_issue` (16); `typecheck exit=0`.

- [ ] **Step 7: Commit**

```bash
cd /Users/mustafajoulani/Desktop/anexa-agents-wt
git add prisma/migrations/20260916100300_action_required_stages/migration.sql src/server/modules/pipeline/__tests__/action-required-stages.itest.ts prisma/seed.ts prisma/seed-clean.ts
...
**Files:**
- Create: `prisma/migrations/20260916100200_agents_seed_rows/migration.sql`
...
cd /Users/mustafajoulani/Desktop/anexa-agents-wt
pnpm exec prisma migrate deploy 2>&1 | grep -E "Applying|Error"
PGOPTIONS="-c search_path=agents_dev" psql "postgresql://anexa:anexa@127.0.0.1:5544/anexa" -v ON_ERROR_STOP=1 -f prisma/migrations/20260916100200_agents_seed_rows/migration.sql
...
Expected: `Applying migration `20260916100200_agents_seed_rows``; the manual re-run prints `INSERT 0 0` twice (nothing new); `Anexa Homes|Hello Agent|system.hello|t|f`; four rows `roofing|agent_needs_human|1`, `roofing|agent_run_failed|1`, `solar|agent_needs_human|1`, `solar|agent_run_failed|1`.
...
```bash
cd /Users/mustafajoulani/Desktop/anexa-agents-wt
git add prisma/migrations/20260916100200_agents_seed_rows/migration.sql prisma/seed.ts prisma/seed-clean.ts
...
- A new migration touching `pipeline_stages`: the positions `20260916100300_action_required_stages` inserts at, and its itest fixture, must be re-checked against it.
