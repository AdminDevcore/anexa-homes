...
/**
 * When a move THROWS partway through a run — the second of several changes,
 * here — everything after it must be discarded too, including a change the
 * gate had already decided to `held`: once the run has failed, a held change
 * can never be approved (gate.ts's own precedence). This lives in its own
 * file because vi.mock is hoisted for the whole file, and only this file
 * needs `moveDeal` itself to misbehave while `resolveChanges` stays real.
 */
...
// moveDeal succeeds on its first call and throws on its second, so the run
// sees exactly one applied move, one that failed outright, and (crucially) a
// third change the gate had already decided to hold.
vi.mock("../apply-changes", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../apply-changes")>();
  let calls = 0;
  return {
    ...actual,
    moveDeal: vi.fn(async (...args: Parameters<typeof actual.moveDeal>) => {
      calls += 1;
      if (calls === 2) throw new Error("portal dropped the connection");
      return actual.moveDeal(...args);
    }),
  };
});
...
beforeAll(async () => {
  companyId = (await db.company.create({ data: { name: "Move Error Co", slug: `move-error-${process.pid}-${Date.now()}` } })).id;
  await db.user.create({
    data: { companyId, email: `owner-${process.pid}@move-error.test`, firstName: "Olive", lastName: "Owner", role: "super_admin", status: "active", passwordHash: "x" },
  });
...
describe("a move fails partway through a run", () => {
  it("applies the first change, discards the one whose move threw, and discards a held change too — none stays held", async () => {
    // Gated, so a move into the (non action-required) "main" stage would
    // normally be held for a human — that is the held change this proves gets
    // discarded once an earlier change in the same run has already failed.
    const a = await agent("test.multimove", {
      requiresHumanGate: true,
      config: {
        moves: [
          { leadId: leadA, to: "side" },
          { leadId: leadB, to: "side" },
    ...
    expect(detail.changes[1]).toMatchObject({ leadId: leadB, outcome: "discarded", note: "Not applied: portal dropped the connection" });
    expect(detail.changes[2]).toMatchObject({ leadId: leadC, outcome: "discarded", note: "Not applied: an earlier change in this run failed." });
