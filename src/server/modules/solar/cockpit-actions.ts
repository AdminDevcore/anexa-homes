...
/**
 * Invite the homeowner to the customer portal.
 *
 * Reuses the existing customer-user link on the lead. Returns a clear error
 * rather than half-working when there is no email on file.
 */
export async function inviteHomeownerAction(leadId: string) {
  const user = await requireUser();
  if (!can(user, "update", "Lead")) return fail("Not allowed.");
  const scope = listScope(user, "Lead") as Prisma.LeadWhereInput;
  const lead = await prisma.lead.findFirst({
    where: { ...scope, companyId: user.companyId, id: leadId },
    select: { id: true, email: true, firstName: true, lastName: true, customerUserId: true },
  });
  if (!lead) return fail("Deal not found.");
  if (!lead.email) return fail("Add an email address to the deal first.");
  if (lead.customerUserId) return fail("This homeowner already has portal access.");
  ...
  try {
    ...
    const tpl = brandedEmailTemplate({
      brand: brand.brand,
      subject: `${brand.brand.companyName}: track your solar project`,
      heading: `Hi ${lead.firstName}, follow your project online`,
      paragraphs: [
        "You can now follow your solar project from start to switch-on — design, permits, install dates and documents, all in one place.",
      ],
      cta: { label: "Open your project", url: `${appUrl}/login` },
    });
    await sendEmail(lead.email, tpl.subject, tpl.text, { fromName: brand.fromName, html: tpl.html });
  } catch {
    return fail("Could not send the invitation email. Check the email settings.");
  }
  ...
  await prisma.activityLog.create({
    data: {
      companyId: user.companyId,
      type: "system",
      message: `${user.fullName} invited the homeowner to the customer portal`,
      actorId: user.userId,
      leadId,
    },
  });
