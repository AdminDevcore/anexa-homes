...
export async function submitDealToLender(input: LenderSubmitInput): Promise<LenderSubmitResult> {
  ...
  const log = (row: Omit<SubmissionLog, "payload">) =>
    recordSubmission(
      ...
      // WHICH SHEET THIS IS. Resolved once, the same way the money was, so the
      // row cannot name a different version from the one it quotes.
      submittedProposalId,
      row,
    );
...
async function recordSubmission(
  ...
  savingBasis: string,
  /**
   * The document every figure above came off — resolved, not passed through.
   * Null only where the deal has no proposal at all and the money fell back to
   * the live `SolarFinance` rows. See `resolveSubmissionProposalId`.
   */
  proposalId: string | null,
...
/**
 * The id of the document a submission speaks for.
 *
 * Deliberately NOT `proposalId ?? null`. Where the caller named one this
 * returns it only after `submissionDocument` has scoped it to this deal and
 * this company — an id from a browser is never a fact about ownership. Where
 * the caller named none, the deal answers for itself and this reports the
 * version it answered WITH, so the recorded row names the sheet the underwriter
 * was actually quoted rather than the request's silence.
 *
 * Null means no proposal exists at all and the money came off the live
 * `SolarFinance` rows — the one route into a submission without a document.
 */
async function resolveSubmissionProposalId(
  leadId: string,
  companyId: string,
  proposalId?: string | null,
): Promise<string | null> {
  const doc = await submissionDocument(leadId, companyId, proposalId);
  return doc?.id ?? null;
}
